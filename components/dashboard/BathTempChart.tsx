...
import { BathTempPoint } from '@/types/manufacturing'

interface Props { data: BathTempPoint[] }

export default function BathTempChart({ data }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="mb-1">
        <h3 className="text-base font-bold text-slate-900">배스 온도 추이</h3>
        <p className="text-xs text-slate-400 mt-0.5">단위: °C</p>
      </div>
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={chartData} margin={{ top: 4, right: 12, left: -4, bottom: 0 }}>
            <defs>
              <linearGradient id="bathFill" x1="0" y1="0" x2="0" y2="1">
            ...
            <Tooltip
              formatter={(v) => [`${Number(v).toFixed(1)} °C`, '배스 온도']}
